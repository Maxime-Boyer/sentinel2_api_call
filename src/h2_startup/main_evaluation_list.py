...
import numpy as np
import pandas as pd
from h2_startup.modules.technical_notation.single_and_multi_resume_comparator.llm_comparator import (
    LLMComparator,
)
...
if __name__ == "__main__":
    ...
    comparators = {
        "single_and_multi_resume_comparator": "single_and_multi_resume_comparator"
    }
    ...
    for ao_id, entreprises_json in folder_structure.items():
        ...
        (
            formated_json_response_list,
            liste_scores_llm_notation,
        ) = llm_comparator.call_comparator(
        ...
        for entreprise in liste_entreprises:
            ...
            (
                ...
                formated_json_response=formated_json_response_list[it],
                score=liste_scores_llm_notation[it],
        ...
        df.to_csv(
            f"/workspace/data/evaluation/evaluation_{comparators['single_and_multi_resume_comparator']}/{ao_id}.csv",
            index=False,
            encoding="utf8",
        )
        df.to_excel(
            f"/workspace/data/evaluation/evaluation_{comparators['single_and_multi_resume_comparator']}/{ao_id}.xlsx"
        )
        ...
        df_per_criteria.to_csv(
            f"/workspace/data/evaluation/evaluation_{comparators['single_and_multi_resume_comparator']}/{ao_id}_per_criteria.csv",
            index=False,
            encoding="utf8",
        )
        df_per_criteria.to_excel(
            f"/workspace/data/evaluation/evaluation_{comparators['single_and_multi_resume_comparator']}/{ao_id}_per_criteria.xlsx",
            index=False,
        )

    save_dict(
        d=dict_results,
        path=f"/workspace/data/evaluation/evaluation_{comparators['single_and_multi_resume_comparator']}/{ao_id}.json",
    )
